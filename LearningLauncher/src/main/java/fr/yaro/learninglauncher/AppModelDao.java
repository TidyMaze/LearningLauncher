...
public interface AppModelDao {
    ...
    @Query("WITH raw AS (\n" +
            "\tSELECT app.label, app.uid, windowQuery.scoreWindow, dowQuery.scoreDOW, allQuery.scoreAll, (ifnull(windowQuery.scoreWindow, 0)*5 + ifnull(dowQuery.scoreDOW,0)*2 + ifnull(allQuery.scoreAll,0))/8 as score, :referenceDate as date\n" +
            "\tFROM\n" +
            "\t\tapp\n" +
            "\t\tLEFT JOIN (\n" +
